    ...
    def process_stories(self, limit: int = None):
        ...
        for story_id, classification_id in classified_stories:
            try:
                ...
                # Get processed words from SQLite
                text_result = self.sqlite_db.execute_query(
                    "SELECT words FROM processed_text WHERE story_id = ?",
                    (story_id,)
                )

                if not text_result or not text_result[0][0]:
                    skipped += 1
                    continue

                words_string = text_result[0][0]

                # Words are already tokenized, split on delimiter
                word_tokens = words_string.split('|') if words_string else []
