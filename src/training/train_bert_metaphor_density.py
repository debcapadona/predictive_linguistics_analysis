"""
train_bert_metaphor_cluster_density.py - Fine-tune BERT to predict metaphor cluster density scores
...
class TimeCompressionDataset(Dataset):
    """Dataset for certainty metaphor cluster density regression"""
...
def load_training_data():
    ...
    pg_cursor.execute("""
        SELECT story_id, metaphor_cluster_density 
        FROM zero_shot_labels 
        WHERE metaphor_cluster_density IS NOT NULL
    """)
...
def train_model(texts, scores, epochs=3, batch_size=16, lr=2e-5):
    """Fine-tune BERT on metaphor cluster density scores"""
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'models/bert_metaphor_cluster_density.pt')
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        print(f"\nText: {text[:60]}...")
        print(f"Predicted metaphor_cluster_density: {score:.3f}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("BERT Metaphor Density Training")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Training complete! Model saved to models/bert_metaphor_cluster_density.pt")
    print("="*60)