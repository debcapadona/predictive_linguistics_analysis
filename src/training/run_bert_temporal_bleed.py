"""
run_bert_temporal_bleed.py - Run trained BERT model on all stories
"""
...
def load_model():
    ...
    checkpoint = torch.load('models/bert_temporal_bleed.pt', map_location=device, weights_only=False)    
    model = BertRegressor()
...
def save_to_postgres(results):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS bert_temporal_bleed (
    ...
    for story_id, score in tqdm(results, desc="Saving to PostgreSQL"):
        cursor.execute("""
            INSERT INTO bert_temporal_bleed (story_id, score)
            VALUES (%s, %s)
...
if __name__ == "__main__":
    print("="*60)
    print("BERT Temporal Bleed - Full Inference")
    print("="*60)
