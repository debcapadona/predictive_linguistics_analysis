"""
train_bert_sacred_profane_ratio.py - Fine-tune BERT to predict sacred profane ratio scores
...
class TimeCompressionDataset(Dataset):
    """Dataset for certainty sacred profane ratio regression"""
...
def load_training_data():
    ...
    pg_cursor.execute("""
        SELECT story_id, sacred_profane_ratio 
        FROM zero_shot_labels 
        WHERE sacred_profane_ratio IS NOT NULL
    """)
...
def train_model(texts, scores, epochs=3, batch_size=16, lr=2e-5):
    """Fine-tune BERT on sacred profane ratio scores"""
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'models/bert_sacred_profane_ratio.pt')
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        print(f"\nText: {text[:60]}...")
        print(f"Predicted sacred_profane_ratio: {score:.3f}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("BERT Sacred Profane Ratio Training")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Training complete! Model saved to models/bert_sacred_profane_ratio.pt")
    print("="*60)