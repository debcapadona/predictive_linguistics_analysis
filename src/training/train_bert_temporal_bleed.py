"""
train_bert_temporal_bleed.py - Fine-tune BERT to predict temporal bleed scores
...
class TimeCompressionDataset(Dataset):
    """Dataset for temporal bleed regression"""
...
def load_training_data():
    ...
    pg_cursor.execute("""
        SELECT story_id, temporal_bleed 
        FROM zero_shot_labels 
        WHERE temporal_bleed IS NOT NULL
    """)
...
def train_model(texts, scores, epochs=3, batch_size=16, lr=2e-5):
    """Fine-tune BERT on temporal bleed scores"""
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        print(f"\nText: {text[:60]}...")
        print(f"Predicted temporal_bleed: {score:.3f}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("BERT Temporal Bleed Training")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Training complete! Model saved to models/bert_temporal_bleed.pt")
    print("="*60)