"""
train_bert_novel_meme_explosion.py - Fine-tune BERT to predict novel meme explosion scores
...
class TimeCompressionDataset(Dataset):
    """Dataset for certainty novel meme explosion regression"""
...
def load_training_data():
    ...
    pg_cursor.execute("""
        SELECT story_id, novel_meme_explosion 
        FROM zero_shot_labels 
        WHERE novel_meme_explosion IS NOT NULL
    """)
...
def train_model(texts, scores, epochs=3, batch_size=16, lr=2e-5):
    """Fine-tune BERT on novel meme explosion scores"""
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'models/bert_novel_meme_explosion.pt')
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        print(f"\nText: {text[:60]}...")
        print(f"Predicted novel_meme_explosion: {score:.3f}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("BERT Novel Meme Explosion Training")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Training complete! Model saved to models/bert_novel_meme_explosion.pt")
    print("="*60)