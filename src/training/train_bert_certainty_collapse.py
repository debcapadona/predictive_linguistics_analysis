"""
train_bert_certainty_collapse.py - Fine-tune BERT to predict certainty collapse scores
...
class TimeCompressionDataset(Dataset):
    """Dataset for certainty collapse regression"""
...
def load_training_data():
    ...
    pg_cursor.execute("""
        SELECT story_id, certainty_collapse 
        FROM zero_shot_labels 
        WHERE certainty_collapse IS NOT NULL
    """)
...
def train_model(texts, scores, epochs=3, batch_size=16, lr=2e-5):
    """Fine-tune BERT on certainty collapse scores"""
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'models/bert_certainty_collapse.pt')
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        print(f"\nText: {text[:60]}...")
        print(f"Predicted certainty_collapse: {score:.3f}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("BERT Certainty Collapse Training")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Training complete! Model saved to models/bert_certainty_collapse.pt")
    print("="*60)