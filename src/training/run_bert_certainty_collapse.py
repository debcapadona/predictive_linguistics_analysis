"""
run_bert_certainty_collapse.py - Run trained BERT model on all stories
"""
...
def load_model():
    ...
    checkpoint = torch.load('models/bert_certainty_collapse.pt', map_location=device, weights_only=False)    
    model = BertRegressor()
...
def save_to_postgres(results):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS bert_certainty_collapse (
    ...
    for story_id, score in tqdm(results, desc="Saving to PostgreSQL"):
        cursor.execute("""
            INSERT INTO bert_certainty_collapse (story_id, score)
            VALUES (%s, %s)
...
if __name__ == "__main__":
    print("="*60)
    print("BERT Certainty Collapse - Full Inference")
    print("="*60)
