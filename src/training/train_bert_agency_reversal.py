"""
train_bert_agency_reversal.py - Fine-tune BERT to predict agency reversal scores
...
class TimeCompressionDataset(Dataset):
    """Dataset for certainty agency reversal regression"""
...
def load_training_data():
    ...
    pg_cursor.execute("""
        SELECT story_id, agency_reversal 
        FROM zero_shot_labels 
        WHERE agency_reversal IS NOT NULL
    """)
...
def train_model(texts, scores, epochs=3, batch_size=16, lr=2e-5):
    """Fine-tune BERT on agency reversal scores"""
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'models/bert_agency_reversal.pt')
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        print(f"\nText: {text[:60]}...")
        print(f"Predicted agency_reversal: {score:.3f}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("BERT Agency Reversal Training")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Training complete! Model saved to models/bert_agency_reversal.pt")
    print("="*60)