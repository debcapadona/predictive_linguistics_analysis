"""
train_bert_time_compression.py - Fine-tune BERT to predict time compression scores
...
class TimeCompressionDataset(Dataset):
    """Dataset for time compression regression"""
...
def load_training_data():
    ...
    pg_cursor.execute("""
        SELECT story_id, time_compression_markers 
        FROM zero_shot_labels 
        WHERE time_compression_markers IS NOT NULL
    """)
...
def train_model(texts, scores, epochs=3, batch_size=16, lr=2e-5):
    """Fine-tune BERT on time compression scores"""
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, 'models/bert_time_compression.pt')
...
def test_model(model, tokenizer, device):
    ...
    for text in test_texts:
        ...
        print(f"\nText: {text[:60]}...")
        print(f"Predicted time_compression: {score:.3f}")
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("BERT Time Compression Training")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("Training complete! Model saved to models/bert_time_compression.pt")
    print("="*60)